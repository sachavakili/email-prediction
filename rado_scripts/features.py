...
class GraphDegreeFeatures(BaseFeatureExtractor):
    ...
    def get_feature(self, edge_idx, edge_info):
        ...
        d_in_p = len([1 for _, _, w in in_source if w['RES'] == 1])
        d_in_n = len([1 for _, _, w in in_target if w['RES'] == -1])
        d_out_p = len([1 for _, _, w in out_source if w['RES'] == 1])
        d_out_n = len([1 for _, _, w in out_target if w['RES'] == -1])
...
class GraphTriadFeatures(BaseFeatureExtractor):
    ...
    def get_feature(self, edge_idx, edge_info):
        ...
        n_in_p_in_p = len(set([u for u, _, w in in_source if w['RES'] == 1])\
                .intersection([u for u, _, w in in_target if w['RES'] == 1]))
        n_in_p_in_n = len(set([u for u, _, w in in_source if w['RES'] == 1])\
                .intersection([u for u, _, w in in_target if w['RES'] == -1]))
        n_in_n_in_p = len(set([u for u, _, w in in_source if w['RES'] == -1])\
                .intersection([u for u, _, w in in_target if w['RES'] == 1]))
        n_in_n_in_n = len(set([u for u, _, w in in_source if w['RES'] == -1])\
                .intersection([u for u, _, w in in_target if w['RES'] == -1]))
        n_in_p_out_p = len(set([u for u, _, w in in_source if w['RES'] == 1])\
                .intersection([v for _, v, w in out_target if w['RES'] == 1]))
        n_in_p_out_n = len(set([u for u, _, w in in_source if w['RES'] == 1])\
                .intersection([v for _, v, w in out_target if w['RES'] == -1]))
        n_in_n_out_p = len(set([u for u, _, w in in_source if w['RES'] == -1])\
                .intersection([v for _, v, w in out_target if w['RES'] == 1]))
        n_in_n_out_n = len(set([u for u, _, w in in_source if w['RES'] == -1])\
                .intersection([v for _, v, w in out_target if w['RES'] == -1]))
        n_out_p_in_p = len(set([v for _, v, w in out_source if w['RES'] == 1])\
                .intersection([u for u, _, w in in_target if w['RES'] == 1]))
        n_out_p_in_n = len(set([v for _, v, w in out_source if w['RES'] == 1])\
                .intersection([u for u, _, w in in_target if w['RES'] == -1]))
        n_out_n_in_p = len(set([v for _, v, w in out_source if w['RES'] == -1])\
                .intersection([u for u, _, w in in_target if w['RES'] == 1]))
        n_out_n_in_n = len(set([v for _, v, w in out_source if w['RES'] == -1])\
                .intersection([u for u, _, w in in_target if w['RES'] == -1]))
        n_out_p_out_p = len(set([v for _, v, w in out_source if w['RES'] == 1])\
                .intersection([v for _, v, w in out_target if w['RES'] == 1]))
        n_out_p_out_n = len(set([v for _, v, w in out_source if w['RES'] == 1])\
                .intersection([v for _, v, w in out_target if w['RES'] == -1]))
        n_out_n_out_p = len(set([v for _, v, w in out_source if w['RES'] == -1])\
                .intersection([v for _, v, w in out_target if w['RES'] == 1]))
        n_out_n_out_n = len(set([v for _, v, w in out_source if w['RES'] == -1])\
                .intersection([v for _, v, w in out_target if w['RES'] == -1]))
